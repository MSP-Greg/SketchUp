# FindSURenderingOptions createss two files, one in columns, the other tab delimited.
# The files list information about SketchUp RenderingOptions.
#
module CreateSUConstantsMD
...
#} Class variables
  ...
  # @return [Void]
  def self._run()
    return unless (@@text_md   = self.file_get_str('Sketchup_Constants_Tmplt.md'))
    return unless (@@text_code = self.file_get_str('Constant_Template_Code.rb'))
...
#    self._run2()
  ...
  def self._run2()
    ...
    @@text_md.gsub!(/SketchUp_Constants_Guide/, "SketchUp #{@@su_major_full} Constants Guide")

    self.file_write("SketchUp_#{@@su_major}_Constants.md", @@text_md, "../../docs/")
  ...
  def self.find_regex(re_con, re_md = nil, cnst_name = nil)
    ...
    if (array.length > 0)
      ...
      re_find_text = /<< #{re_md} >>/
      @@text_md.sub!(re_find_text, "**\* Constants not defined in SketchUp #{@@su_major_full} \***") if re_md
      array
    end
  end
  ...
  # used to delete rendering option constants
  def self.rendering_options_del(array, col, ns = '' )
    array.each { |row|
      next if row[col].match(/[^A-Za-z0-9_]/)
      t = /^#{ns}#{row[col]}.+?$/
      @@constants.sub!(/^#{ns}#{row[col]}.+?$/, '')
    }
  end
  ...
  def self.rendering_options_done()
    ...
    hdr_row = ['RenderingOptions<br/>key', '<br/>RO value.class', 'Observer<br/>constant (type)']
    alignment = ['L', 'L', 'L']
    self.rendering_options_del(@@a_ro_table, 2, 'Sketchup::RenderingOptions::')
    ...
    if (@@a_no_fire.length > 0)
      hdr_row = ['RenderingOptions<br/>key', '<br/>RO value.class']
      alignment = ['L', 'L']
...
# odd cases  showRubyPanel
    ...
    if (array.length > 0)
      ...
      table = self.standard_table(array, 'cmd_numeric', 'Numeric<br/>Constant')
    end
  end
  ...
  # creates table text from an array
  ...
  # @return [string]
  # &#160;
  def self.create_table1(array, hdr_row, alignment)
    rows = array.length - 1
    columns = hdr_row.length - 1
